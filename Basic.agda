...
postulate
  ...
  rev : ∀ {u} {A : Type u} (a b : A) → C (Path A a b) (λ _ → Path A b a)
  com : ∀ {u} {A : Type u} (a b c : A) → C (Path A a b) (λ _ → C (Path A b c) (λ _ → Path A a c))
...
_⁻¹ {a = a} {b = b} = ap (rev a b)

_⬝_ : ∀ {u} {A : Type u} {a b c : A} → Path A a b → Path A b c → Path A a c
_⬝_ {a = a} {b = b} {c = c} p q = ap (ap (com a b c) p) q
